...
```{r}
ggplot(survey_df, aes(fill = Question, x = Characteristics, y = US1)) +
...
```{r}
ny_df <- survey_df %>%
  select(Question, Characteristics, contains("NY")) %>%
  filter(Question == "Age")

ggplot(ny_df, aes(fill = Characteristics, x = Characteristics, y = NY1)) +
  geom_bar(position='dodge', stat='identity') 
```
